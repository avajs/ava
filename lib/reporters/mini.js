	...
	if (this.failCount > 0) {
		runStatus.errors.forEach(function (test, index) {
			...
			if (test.error.source) {
				...
				var errorPath = path.join(this.options.basePath, test.error.source.file);
				var excerpt = codeExcerpt(errorPath, test.error.source.line, {maxWidth: process.stdout.columns});
				status += indentString(excerpt, 2) + '\n';
			}
