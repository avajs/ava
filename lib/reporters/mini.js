	...
	if (this.failCount > 0) {
		runStatus.errors.forEach(function (test, index) {
			...
			if (test.error.source) {
				var errorPath = test.error.source.file + ':' + test.error.source.line;
				status += '  ' + colors.errorSource(errorPath) + '\n\n';
				status += indentString(codeExcerpt(path.join(this.options.basePath, test.error.source.file), test.error.source.line, {maxWidth: process.stdout.columns}), 2) + '\n';
			}
	...
	if (this.rejectionCount > 0 || this.exceptionCount > 0) {
		runStatus.errors.forEach(function (err) {
			...
			if (err.type === 'exception' && err.name === 'AvaError') {
				...
				var errorTitle = err.name ? description[0] : 'Threw non-error: ' + description[0];
