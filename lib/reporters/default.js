...
export default class Reporter {
	...
	writeTestSummary(event) {
		// Icon indicates matched expectations vs. not.
		// Color indicates passed-as-expected vs. not (expected/unexpected fail or unexpected pass).
		if (event.type === 'hook-failed' || event.type === 'test-failed') {
			const description = event.knownFailing ? 'UNEXPECTED PASS' : 'FAIL';
			this.write(`${colors.error(`${figures.cross} ${description}:`)} ${this.prefixTitle(event.testFile, event.title)} ${colors.error(event.err.message)}`);
		} else if (event.knownFailing) {
			this.write(`${colors.error(figures.tick + ' EXPECTED FAIL:')} ${this.prefixTitle(event.testFile, event.title)}`);
		} else {
			const duration = event.duration > this.durationThreshold ? colors.duration(' (' + prettyMs(event.duration) + ')') : '';
			this.write(`${colors.pass(figures.tick + ' PASS:')} ${this.prefixTitle(event.testFile, event.title)}${duration}`);
		}
