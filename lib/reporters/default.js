...
class Reporter {
	...
	startRun(plan) {
		...
		this.emptyParallelRun = this.isVerbose ?
			plan.status.emptyParallelRun :
			this.emptyParallelRun;
		...
		const stream = this.isVerbose ? this.lineWriter : this.reportStream;
		const columns = !stream.columns && this.isVerbose ? 80 : stream.columns;

		if (this.watching && plan.runVector > 1) {
			stream.write(chalk.gray.dim('\u2500'.repeat(columns)) + os.EOL);
		}
	...
	consumeStateChange(event) {
		const fileStats =
			this.stats && event.testFile ?
				this.stats.byFile.get(event.testFile) :
				null;
		const write = this.isVerbose ?
			this.lineWriter.writeLine.bind(this.lineWriter) :
			this.writeWithCounts.bind(this);
		const STATES = {
			'hook-failed': () => {
			...
			'internal-error': () => {
				...
				if (event.testFile) {
					...
					write(colors.error(`${figures.cross} Internal error`));
				}
			...
			'hook-finished': () => {
				if (!this.isVerbose || event.logs.length === 0) {
					return;
				}

				this.lineWriter.writeLine(
					`  ${this.prefixTitle(event.testFile, event.title)}`
				);
			...
			'selected-test': () => {
				...
				if (event.skip) {
					this.lineWriter.writeLine(
						colors.skip(`- ${this.prefixTitle(event.testFile, event.title)}`)
					);
				} else if (event.todo) {
					this.lineWriter.writeLine(
						colors.todo(`- ${this.prefixTitle(event.testFile, event.title)}`)
					);
				}
			},
			'uncaught-exception': () => {
				...
				this.lineWriter.writeLine(
					colors.title(
						`Uncaught exception in ${this.relativeFile(event.testFile)}`
					)
				);
				this.lineWriter.writeLine();
				this.writeErr(event);
			...
			'unhandled-rejection': () => {
				...
				this.lineWriter.writeLine(
					colors.title(
						`Unhandled rejection in ${this.relativeFile(event.testFile)}`
					)
				);
				this.lineWriter.writeLine();
				this.writeErr(event);
						...
						this.lineWriter.writeLine(
							colors.error(
								`${figures.cross} ${this.relativeFile(
									event.testFile
								)} exited with a non-zero exit code: ${event.nonZeroExitCode}`
							)
						);
					} else {
						this.lineWriter.writeLine(
			...
			'worker-finished': () => {
				if (
					!event.forcedExit &&
					!this.filesWithMissingAvaImports.has(event.testFile)
				) {
					if (fileStats.declaredTests === 0) {
						if (!this.isVerbose) {
							this.filesWithoutDeclaredTests.add(event.testFile);
						...
						fileStats.selectingLines &&
						fileStats.selectedTests === 0
					) {
						if (!this.isVerbose)	{
						...
						this.lineWriter.writeLine(
							colors.error(
								...
								)} remaining in ${this.relativeFile(event.testFile)}`
							)
						);
					}
				}
	...
	writeErr(event) {
		if (
			...
			this.lineWriter.writeLine(
				colors.errorStack(trimOffNewlines(event.err.object.diagnosticText))
			);
			this.lineWriter.writeLine();
			return;
		}
		...
		if (event.err.source) {
			this.lineWriter.writeLine(
				colors.errorSource(
					`${this.relativeFile(event.err.source.file)}:${event.err.source.line}`
				)
			);
	...
	writeTestSummary(event) {
		if (event.type === 'hook-failed' || event.type === 'test-failed') {
			if (this.isVerbose) {
				this.lineWriter.writeLine(
					`${colors.error(figures.cross)} ${this.prefixTitle(
						event.testFile,
						event.title
					)} ${colors.error(event.err.message)}`
				);
			} else {
			...
			if (this.isVerbose) {
				...
				this.writeWithCounts(
					`${colors.error(this.prefixTitle(event.testFile, event.title))}`
				);
			}
		} else if (this.isVerbose) {
			const duration =
				event.duration > this.durationThreshold ?
					colors.duration(' (' + prettyMs(event.duration) + ')') :
					'';
	...
	writeFailure(event) {
		this.lineWriter.writeLine(
			`${colors.title(this.prefixTitle(event.testFile, event.title))}`
		);
	...
	endRun() {// eslint-disable-line complexity
		...
		if (this.matching && this.stats.selectedTests === 0) {
			this.lineWriter.writeLine(
				colors.error(
					`${figures.cross} Couldn’t find any matching tests` + firstLinePostfix
				)
			);
			this.lineWriter.writeLine();
			return;
		...
		if (this.failures.length > 0) {
			...
			for (const event of this.failures) {
				...
				if (event !== lastFailure) {
					this.lineWriter.writeLine();
					this.lineWriter.writeLine();
				} else if (!this.isVerbose && writeTrailingLines) {
					this.lineWriter.writeLine();
					this.lineWriter.writeLine();
				}
		...
		if (this.isVerbose && this.stats.parallelRuns) {
			const {
				currentFileCount,
				currentIndex,
				totalRuns
