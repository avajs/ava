...
class Reporter {
	...
	startRun(plan) {
		...
		this.emptyParallelRun = this.verbose ?
		...
		const stream = this.verbose ? this.lineWriter : this.reportStream;
		const columns = !stream.columns && this.verbose ? 80 : stream.columns;
