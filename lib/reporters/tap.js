...
class TapReporter {
	...
	writeLogs(evt, flags = {error: false, title: ''}) {
		let formattedTitle = flags.title || this.prefixTitle(evt.testFile, evt.title);
		if (flags.error) {
			...
			case 'worker-finished':
				if (!evt.forcedExit && !this.filesWithMissingAvaImports.has(evt.testFile)) {
					if (fileStats.declaredTests === 0) {
						...
						this.writeLogs(evt, {title: `${fileStats.remainingTests} ${plur('test', fileStats.remainingTests)} remaining in ${path.relative('.', evt.testFile)}`});
					}
				}
