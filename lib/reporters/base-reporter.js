...
class BaseReporter {
	...
	consumeStateChange(event) {
		const STATES = {
			...
			verbose: {
				...
				'uncaught-exception': () => {
					...
					this.lineWriter.writeLine(
						colors.title(`Uncaught exception in ${this.relativeFile(event.testFile)}`)
					);
					this.lineWriter.writeLine();
					this.writeErr(event);
				},
		...
		if (STATES[event.type]) {
			STATES[event.type](fileStats);
		} else if (STATES[this.mode][event.type]) {
			STATES[this.mode][event.type](fileStats);
		}
	}
	...
	writeTestSummary(event) {
		if (this.mode === 'verbose') {
			...
			this.writeLogs(event);
		} else if ((event.type === 'hook-failed') || (event.type === 'test-failed')) {
			this.writeWithCounts(`${this.prefixTitle(event.testFile, event.title)}`);
		} else if (event.knownFailing) {
			this.writeWithCounts(
