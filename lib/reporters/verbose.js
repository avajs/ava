...
class VerboseReporter {
	...
	endRun() { // eslint-disable-line complexity
		...
		if (this.stats.splitTests) {
			const {index, total, start, end} = this.stats.splitTests;
			this.lineWriter.writeLine(colors.information(`Ran ${end - start} test ${plur('file', end - start)} out of ${this.stats.files} for job ${index + 1} of ${total}`));
			this.lineWriter.writeLine();
		}
