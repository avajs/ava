...
class VerboseReporter {
	...
	removeTestRunning(evt) {
		console.log('removeTestRunning', '🍇');
		this.runningTestsCount--;
		this.runningTestFiles.get(evt.testFile).delete(evt.title);
	}

	runningTestSummary() {
		console.log('runningTestSummary', '🍇');
		if (this.runningTestsCount === 0) {
			return 'NO TESTS 🍓';
		}
		let smy = `\n${this.runningTestsCount} tests still running:`;
		this.runningTestFiles.forEach((tests, file) => {
			for (const title of this.runningTestFiles.get(file)) {
