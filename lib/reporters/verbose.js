...
class VerboseReporter {
	...
	endRun() { // eslint-disable-line complexity
		...
		if (this.stats.parallelRuns) {
			const {count, index, total} = this.stats.parallelRuns;
			this.lineWriter.writeLine(colors.information(`Ran ${count} test ${plur('file', count)} out of ${this.stats.files} for job ${index + 1} of ${total}`));
			this.lineWriter.writeLine();
		}
