...
VerboseReporter.prototype.test = function (test, runStatus) {
	...
	if (test.failing) {
		return ' ' + colors.error(test.title + ' ' + colors.error(test.error.message));
	}
...
VerboseReporter.prototype.finish = function (runStatus) {
	...
	var lines = [
		runStatus.knownFailureCount > 0 ? '  ' + colors.error(runStatus.knownFailureCount, plur('test', runStatus.knownFailureCount), 'known failure') : '',
