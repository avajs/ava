...
class VerboseReporter {
	...
	removeTestRunning(file, title) {
		if (!this.runningTestFiles.get(file) || !this.runningTestFiles.get(file).has(title)) {
			return;
		}
		this.runningTestsCount--;
		this.runningTestFiles.get(file).delete(title);
		if (this.runningTestFiles.get(file).size === 0) {
			this.runningTestFiles.delete(file);
		}
	}

	runningTestSummary(evt) {
		let smy = '';
		...
		for (const timedOutFile of evt.timedOutWorkerFiles) {
			...
			if (currentFile) {
				...
				for (const title of currentFile) {
					smy += `\n${figures.circleDotted} ${this.prefixTitle(timedOutFile, title)}`;
					this.removeTestRunning(timedOutFile, title);
				}
			}
		}
		return smy;
