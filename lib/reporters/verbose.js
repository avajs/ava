...
class VerboseReporter {
	...
	consumeStateChange(evt) { // eslint-disable-line complexity
		...
		switch (evt.type) {
			...
			case 'selected-test':
				...
				break;
			case 'snapshot-error':
				this.snapshotsNotUpdated.push(evt);
				break;
			case 'stats':
	...
	endRun() { // eslint-disable-line complexity
		...
		if (this.snapshotsNotUpdated.length > 0) {
			this.lineWriter.writeLine(colors.title('Refused to update snapshots for the following test files:'));
			this.lineWriter.writeLine();
			for (const evt of this.snapshotsNotUpdated) {
				this.lineWriter.writeLine(`${path.relative(process.cwd(), evt.err.source.file)}: ${evt.err.message}`);
			}
		}
