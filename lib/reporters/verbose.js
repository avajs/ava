		...
		switch (evt.type) {
			...
			case 'worker-finished':
				if (!evt.forcedExit && !this.filesWithMissingAvaImports.has(evt.testFile)) {
					...
					} else if (fileStats.runOnlySelectedLines && fileStats.selectedTests === 0) {
						this.lineWriter.writeLine(colors.error(`${figures.cross} No tests selected by line numbers in ${this.relativeFile(evt.testFile)}`));
