...
VerboseReporter.prototype.test = function (test) {
	...
	if (test.error) {
		output = '  ' + colors.error(figures.cross) + ' ' + test.title + ' ' + colors.error(test.error.message);
		this.write(output);
		return;
	}

	if (test.todo) {
		output = '  ' + colors.todo('- ' + test.title);
		this.write(output);
		return;
	} else if (test.skip) {
		output = '  ' + colors.skip('- ' + test.title);
		this.write(output);
		return;
	}

	if (test.failing) {
		output = '  ' + colors.error(figures.tick) + ' ' + colors.error(test.title);
		this.write(output);
		return;
	}
	...
	output = '  ' + colors.pass(figures.tick) + ' ' + test.title + duration;
	this.write(output);
};
...
VerboseReporter.prototype.unhandledError = function (err) {
	...
	if (err.type === 'exception' && err.name === 'AvaError') {
		output = colors.error('  ' + figures.cross + ' ' + err.message);
		this.write(output);
		return;
	}
	...
	output = colors.error(types[err.type] + ':', err.file) + '\n';
