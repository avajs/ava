...
class Test {
	constructor(options) {
		...
		this.runAttempt = async (title, fn) => {
			...
			const {deferredSnapshotRecordings, error, logs, passed, snapshotCount} = await attempt.run();
			const errors = error ? [error] : [];
			return {deferredSnapshotRecordings, errors, logs, passed, snapshotCount, startingSnapshotCount};
		};
	...
	finishAttempt({assertCount, commit, deferredSnapshotRecordings, errors, logs, passed, retainLogs, snapshotCount, startingSnapshotCount}) {
		if (this.finishing) {
			if (commit) {
