...
class ExecutionContext extends assert.Assertions {
	constructor(test) {
		...
		this.try = async (...attemptArgs) => {
			const {args, implementations, receivedImplementationArray, specifiedTitle} = parseTestArgs(...attemptArgs);

			if (implementations.length === 0) {
				throw new TypeError('Expected an implementation.');
			}

			const attemptPromises = implementations.map(async implementation => {
				const attemptTitle = implementation.title ?
				...
				test.addPendingAttemptAssertion();

				const attemptTest = test.forAttempt(attemptTitle, t => implementation(t, ...args));

				const ret = await attemptTest.run();
			...
			return receivedImplementationArray ? results[0] : results;
		};
	}
...
class Test {
	constructor(options) {
		...
		this.forAttempt = (title, fn) => {
			...
			const opts = {
				...options,
				contextRef: this.contextRef.copy(),
	...
	verifyAssertions() {
		...
		if (this.failWithoutAssertions) {
			if (this.calledEnd) {
				return; // `t.end()` is treated as an assertion.
			}
