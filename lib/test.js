...
class ExecutionContext extends assert.Assertions {
	...
	log(...inputArgs) {
		...
		if (args.length > 0) {
			testMap.get(this).addLog(args.join(' '));
		}
	}

	plan(count) {
		const test = testMap.get(this);
		test.plan(count, captureStack(test.plan)); // Or captureStack(this.plan) ?
	}

	timeout(ms) {
		testMap.get(this).timeout(ms);
	}
	...
	_pass() {
		testMap.get(this).countPassedAssertion();
	}

	_pending(promise) {
		testMap.get(this).addPendingAssertion(promise);
	}

	_fail(err) {
		testMap.get(this).addFailedAssertion(err);
	}
	...
	_compareWithSnapshot(options) {
		return testMap.get(this).compareWithSnapshot(options);
	}
