	...
	constructor(test) {
		...
		this.try = async (...attemptArgs) => {
			...
			const attemptPromises = implementations.map(implementation => {
				...
				const {assertCount, passed, errors, logs} = await test.runAttempt(title, t => implementation(t, ...args));
...
class Test {
	constructor(options) {
		...
		this.runAttempt = async (title, fn) => {
			...
			const {passed, error, logs} = await attempt.run();
			const errors = error ? [error] : [];
			return {passed, errors, logs};
		};
	...
	finishAttempt({assertCount, commit, passed, errors, logs, retainLogs}) {
		if (this.finishing) {
			if (commit) {
