...
function tryTest(title, fn, ...args) {
	...
	return running.then(ret => {
		...
		return {
			...
			commit: ({retainLogs = true} = {}) => {
				...
				committed = true;
				this.countAttemptAssertion({inc: 1, result: ret, retainLogs});
			},
			discard: ({retainLogs = false} = {}) => {
				...
				discarded = true;
				this.countAttemptAssertion({inc: 0, result: ret, retainLogs});
			}
		};
	});
}
