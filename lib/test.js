...
function tryTest(fn, ...args) {
	let committed = false;
	let discarded = false;

	const countAttempt = result => {
		if (result.passed) {
			this.countFinishedAttemptAssertion(result.inc);
		} else {
			this.countFailedAttemptAssertion(result.error, result.inc);
		}
	};

	const saveLogs = (retainLogs, logs) => {
		if (retainLogs) {
	...
	const finishedRunning = running.then(ret => {
		...
		return {
			...
			commit: ({retainLogs = true} = {}) => {
				...
				if (discarded) {
					this.saveFirstError(new Error('Can\'t commit the result that was previously discarded'));
					return;
				}
				committed = true;

				countAttempt({passed, error, inc: 1});
				saveLogs(retainLogs, logs);
			},
			discard: ({retainLogs = false} = {}) => {
				if (committed) {
					this.saveFirstError(new Error('Can\'t discard the result that was previously committed'));
					return;
				}
				if (discarded) {
					return;
				...
				countAttempt({passed, error, inc: 0});
				saveLogs(retainLogs, logs);
			}
		};
	});
	...
	finishedRunning.discard = () => {
		if (committed) {
			this.saveFirstError(new Error('Attempt is already committed'));
			return;
		}
		discarded = true;
...
class Test {
	...
	countFinishedAttemptAssertion(increment) {
		if (this.finishing) {
			this.saveFirstError(new Error('Attempt is complete, but the test has already finished'));
		}

		this.assertCount += increment;
		this.decrementAttemptAssertion();
	}

	countFailedAttemptAssertion(error, increment) {
		this.countFinishedAttemptAssertion(increment);
		this.saveFirstError(error);
	}
