...
class Test {
	constructor(options) {
		...
		this.registerUniqueTitle = registerUniqueTitle;
		...
		const {inline: deferRecording} = this.annotations;
		...
		this.compareWithSnapshot = ({expected, id, message}) => {
			...
			const {record, ...result} = compareTestSnapshot({belongsTo, deferRecording, expected, index, label});
			if (record) {
		...
		this.runAttempt = async (title, fn) => {
			...
			const attempt = new Test({
				...options,
				annotations: {...annotations, callback: false, failing: false, inline: true},
