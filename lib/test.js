...
class Test {
	constructor(options) {
		...
		this.compareWithSnapshot = ({expected, id, message}) => {
			...
			const {record, ...result} = options.compareTestSnapshot({belongsTo, deferRecording, expected, index, label, taskIndex: this.metadata.taskIndex, snapIndex: this.snapshotCount});
			if (record) {
