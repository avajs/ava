	...
	endCallback(err, stack) {
		...
		if (err) {
			this.saveFirstError(new assert.AssertionError({
				...
				stack,
				values: [formatAssertError.formatWithLabel('Callback called with an error:', err)]
			}));
		}
		...
		if (!result.ok) {
			if (!this.detectImproperThrows(result.error)) {
				this.saveFirstError(new assert.AssertionError({
					...
					values: [formatAssertError.formatWithLabel('Error thrown in test:', result.error)]
				}));
			}
			return this.finish();
		}
					...
					.catch(err => {
						if (!this.detectImproperThrows(err)) {
							this.saveFirstError(new assert.AssertionError({
								...
								stack: err instanceof Error && err.stack,
								values: [formatAssertError.formatWithLabel('Rejected promise returned by test. Reason:', err)]
							}));
						}
					})
