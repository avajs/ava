...
class Api extends Emittery {
	...
	async run({files: selectedFiles = [], filter = [], runtimeOptions = {}} = {}) {
		...
		try {
			...
			if (!isCi) {
				Scheduler.failingTestsFirst(this.options.projectDir, selectedFiles);
			}
			...
			runStatus.on('stateChange', record => {
				if (record.testFile && !timedOutWorkerFiles.has(record.testFile) && record.type !== 'test-failed') {
				...
				if (record.type === 'test-failed' && !failedTestFiles.find(file => file === record.testFile)) {
					failedTestFiles.push(record.testFile);
				}
			});
			...
			this._storeFailedTestFiles(failedTestFiles);
		} catch (error) {
	...
	_storeFailedTestFiles(files) {
		if (isCi) {
			return;
		}

		const filePath = Scheduler.prepareTestResultsFile(this.options.projectDir);
		if (files.length === 0) {
			Scheduler.deleteFile(filePath);
			return;
		}

		const jsonFiles = JSON.stringify(files);
		Scheduler.store(filePath, jsonFiles);
	}
}
