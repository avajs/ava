...
class Api extends Emittery {
	...
	async run(files = [], runtimeOptions = {}) {
		...
		try {
			...
			if (files.length === 0) {
				files = await globs.findTests({cwd: this.options.projectDir, ...apiOptions.globs});
			}
		} catch (error) {
			files = [];
		...
		try {
			...
			const babelState = babelProvider === undefined ? null : await babelProvider.compile({cacheDir, files});
