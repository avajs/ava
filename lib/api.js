...
class Api extends Emittery {
	...
	async run({files: selectedFiles = [], filter = [], runtimeOptions = {}} = {}) { // eslint-disable-line complexity
		...
		try {
			...
			selectedFiles = scheduler.failingTestsFirst(selectedFiles, this._createCacheDir(), this.options.cacheEnabled);
			...
			scheduler.storeFailedTestFiles(runStatus, this._cacheDir, this.options.cacheEnabled);
		} catch (error) {
