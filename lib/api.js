...
const ForkTestPool = require('./test-pools/fork-test-pool');
const SharedForkTestPool = require('./test-pools/shared-fork-test-pool');
const SingleProcessTestPool = require('./test-pools/single-process-test-pool');
...
class Api extends Emittery {
	...
	async run(files, runtimeOptions = {}) {
		...
		try {
			...
			await testPool.run(files, runtimeOptions);
		} catch (error) {
