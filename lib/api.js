...
import {generateSeed, shuffleFiles} from './random-order.js';
...
export default class Api extends Emittery {
	constructor(options) {
		...
		// Generated once, so that watch mode reuses the same order.
		this.randomSeed = this.options.randomizeFileOrder ? this.options.seed ?? generateSeed() : null;
		...
		try {
			if (this.options.parallelRuns) {
				...
				if (this.randomSeed !== null) {
					selectedFiles = shuffleFiles(selectedFiles, this.randomSeed);
				}
