	...
	async run({files: selectedFiles = [], filter = [], runtimeOptions = {}} = {}) { // eslint-disable-line complexity
		...
		try {
			if (this.options.parallelRuns) {
				...
				selectedFiles = selectedFiles.sort(this.options.parallelRunsComparator);
			...
			await pMap(selectedFiles, async file => {
				...
				// Removing `providers` and `parallelRunsComparator` fields because they cannot be transferred to the worker threads.
				const {providers, parallelRunsComparator, ...forkOptions} = apiOptions;
				const options = {
					...forkOptions,
					providerStates,
					lineNumbers,
