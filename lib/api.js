...
class Api extends Emittery {
	constructor(options) {
		...
		this.options = {match: [], lineNumbers: {}, ...options};
	...
	async run({files: selectedFiles = [], filter = [], runtimeOptions = {}} = {}) {
		...
		try {
			...
			await Bluebird.map(selectedFiles, async file => {
				...
				const lineNumbers = apiOptions.lineNumbers[file] || [];
				const options = {
					...apiOptions,
					babelState,
					...
					lineNumbers,
					runOnlySelectedLines: lineNumbers.length > 0
				};
