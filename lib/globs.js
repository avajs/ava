...
export function applyTestFileFilter({cwd, expandDirectories = true, filter, testFiles}) {
	...
	if (unmatchedPatterns.size === 0 || !expandDirectories) {
		return [...selected];
	}

	const directories = globDirectoriesSync(cwd, [...unmatchedPatterns]);
