...
function wantsStage4(userOptions, projectDir) {
	...
	return userOptions.testOptions.presets.every(array => {
...
function build(projectDir, cacheDir, userOptions, compileEnhancements) {
	...
	const loadOptions = filename => {
		...
		if (ensureStage4 && !testOptions.presets.some(containsStage4)) { // eslint-disable-line unicorn/no-fn-reference-in-iterator
			// Apply last.
			testOptions.presets.unshift(createConfigItem('../stage-4', 'preset'));
		}

		if (compileEnhancements && !testOptions.presets.some(containsTransformTestFiles)) { // eslint-disable-line unicorn/no-fn-reference-in-iterator
			// Apply first.
