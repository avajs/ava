...
const boundJson = require('./bound-builtins').json;
...
function hashPartialTestConfig({babelrc, config, options: {plugins, presets}}, projectDir, pluginAndPresetHashes) {
	...
	if (babelrc) {
		...
		if (filename === 'package.json') {
			inputs.push(boundJson.stringify(pkgConf.sync('babel', {cwd: path.dirname(filename)})));
		} else {
...
function build(projectDir, cacheDir, userOptions, compileEnhancements) {
	...
	return filename => {
		...
		if (map) {
			...
			writeFileAtomic.sync(mapPath, boundJson.stringify(map));
