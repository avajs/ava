...
function extractSource(stack, testFile) {
	...
	stack.split('\n').forEach(line => {
		try {
			...
			// Assume the CWD is the project directory (see function `buildSource()`
			// and assume `testFile` is a absolute path.
			if (callSite.file === path.relative(process.cwd(), testFile)) {
				result = callSite;
			}
		} catch (_) {
		}
	...
	if (result) {
		return {
			...
			file: path.resolve(process.cwd(), result.file),
			line: result.line
		};
	}
