...
function extractSource(stack, shouldHighlightAssert) {
	if (!stack) {
		return null;
	}
	...
	return shouldHighlightAssert ?
		stackUtils.parseLine(lines[lines.length - 1]) :
...
function trySerializeError(err, shouldBeautifyStack) {
	...
	const retval = {
		...
		source: buildSource(extractSource(stack, err.shouldHighlightAssert)),
		stack
	};
