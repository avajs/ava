...
// Ignore, assume this only installs babel-register which is now built-in.
// (opts.require || []).forEach(require);

var fs = require('fs');
var currentlyUnhandled = require('currently-unhandled')();
var cacheWorker = require('./babel/cache-worker');
...
cacheWorker.runTest({
	testPath: testPath,
	babelConfig: opts.babel,
	loadWorker: function () {
		return require('./babel/bundle').loadWorker({
			cachedData: fs.readFileSync(opts.bundle.cachedDataFile),
			code: fs.readFileSync(opts.bundle.codeFile)
		});
	}
});
