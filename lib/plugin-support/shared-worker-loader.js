...
function broadcastMessage(data) {
	// TODO: broadcasting can be done directly here
	// sice we have a direct channel with each testWorker.
	// Not sure what to return though.
	const id = nextMessageId();
	const serializedData = [...v8.serialize(data)];
	for (const trackedWorker of activeTestWorkers.values()) {
		trackedWorker.instance.port.postMessage({
			type: 'broadcast',
			messageId: id,
			serializedData
		});
		...
		negotiateProtocol(supported) {
			...
			const setupPort = port => {
				port.on('message', message => {
					...
					setImmediate(() => events.emit('message', message.ava));
				});
			};
