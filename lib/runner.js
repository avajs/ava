...
class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.snapshots = snapshotManager.lazy(() => {
			const snapshots = snapshotManager.load({
				file: this.file,
				fixedLocation: this.snapshotDir,
				projectDir: this.projectDir,
				recordNewSnapshots: this.recordNewSnapshots,
				updating: this.updateSnapshots && !this.runOnlyExclusive
			});
			snapshots.setOrdering(this.taskOrder);
		...
		this.chain = createChain((metadata, testArgs) => { // eslint-disable-line complexity
			...
			if (metadata.todo) {
				...
				for (const implementation of implementations) {
					...
					this.taskOrder.push(title);

					if (task.metadata.skipped) {
						this.snapshots.skipBlock(task.title);
	...
	skipSnapshot(options) {
		this.shouldClean = false;
		this.snapshots.skipSnapshot(options);
	}

	saveSnapshotState() {
		if (this.updateSnapshots && this.runOnlyExclusive) {
			return {cannotSave: true};
		}

		if (!this.snapshots.lazy) {
			return {touchedFiles: this.snapshots.save()};
		}

		if (this.updateSnapshots && this.shouldClean) {
