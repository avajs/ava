...
Runner.prototype.addSerialTest = function (title, cb) {
	this.stats.testCount++;
	this.testsSerial.push(new Test(title, cb));
};
...
Runner.prototype.concurrent = function (cb) {
	each(this.tests, function (test, i, next) {
...
Runner.prototype.serial = function (cb) {
	eachSerial(this.testsSerial, function (test, next) {
		test.run(function (err) {
			if (err) {
				this.stats.failCount++;
			}
...
Runner.prototype.run = function (cb) {
	if (this.testsSerial.length > 0 && this.tests.length === 0) {
