...
class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.chain = createChain((metadata, testArgs) => { // eslint-disable-line complexity
			...
			if (metadata.todo) {
				...
				if (metadata.skipped && this.updateSnapshots) {
					this.updateSnapshots = null;
					this.errorUpdatingSnapshots = new SnapshotError('Tests can not be skipped when snapshots should be updated.');
				}

				if (metadata.exclusive && this.updateSnapshots) {
					this.updateSnapshots = null;
					this.errorUpdatingSnapshots = new SnapshotError('`only` tests are not allowed when snapshots should be updated.');
				}
	...
	saveSnapshotState() {
		if (this.errorUpdatingSnapshots) {
			throw this.errorUpdatingSnapshots;
		}
