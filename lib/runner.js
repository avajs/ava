...
export default class Runner extends Emittery {
	...
	async runMultiple(runnables, serial=false) {
		let allPassed = true;
		if (serial) {
			// TODO: Implement support for serial saw that we pass this.serial
		}
		const storedResults = [];
	...
	async runHooks(tasks, contextRef, {titleSuffix="", testPassed=false} = {}) {
