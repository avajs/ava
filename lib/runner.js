...
const {initializeLineNumberSelector} = require('./line-numbers');
...
class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.lineNumbers = initializeLineNumberSelector(options.file, options.lineNumbers);
		...
		this.chain = createChain((metadata, testArgs) => { // eslint-disable-line complexity
			...
			if (this.lineNumbers) {
				metadata.selected = this.lineNumbers.isSelected();
			}
	...
	async start() {
		...
		for (const task of this.tasks.serial) {
			...
			if (this.lineNumbers && !task.metadata.selected) {
				continue;
			}
