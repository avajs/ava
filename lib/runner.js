		...
		this.chain = createChain((metadata, testArgs) => { // eslint-disable-line complexity
			...
			if (metadata.todo) {
				...
				if (metadata.skipped && this.updateSnapshots) {
					throw new TypeError('Tests can not be skipped when snapshots should be updated.');
				}

				if (metadata.exclusive && this.updateSnapshots) {
					throw new TypeError('`only` tests are not allowed when snapshots should be updated.');
				}
