...
class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.skippingHooks = false;
		this.hasKnownFailingTests = false;
		// Set by lib/worker/subprocess.js if the test file fails to load
			...
			if (metadata.todo) {
				...
				for (const implementation of implementations) {
					...
					if (metadata.type === 'test') {
						...
						if (task.metadata.failing) {
							this.hasKnownFailingTests = true;
						}
	...
	async saveSnapshotState() {
		...
		if (this.updateSnapshots) {
			if (
				...
				!this.skippingHooks &&
				!this.skippingTests &&
				!this.hasKnownFailingTests &&
				!this.runOnlyExclusive &&
				!this.checkSelectedByLineNumbers &&
				!this.declarationFailed
			) {
				await snapshotManager.cleanSnapshots({
					file: this.file,
					fixedLocation: this.snapshotDir,
					projectDir: this.projectDir
				});
	...
	async runSingle(runnable) {
		...
		this.anyFailure = this.anyFailure || !result.passed;
