...
export default class Runner extends Emittery {
	...
	async start() {
		...
		// When a seed is provided, shuffle concurrent tests using a seeded PRNG
		// for reproducible random ordering. Serial tests (.serial) always preserve
		// source order.
		if (this.seed !== undefined && concurrentTests.length > 1) {
			Runner.seededShuffle(concurrentTests, this.seed);
		}
	...
	// Fisher-Yates shuffle using a Park-Miller LCG seeded PRNG.
	// Mutates the array in place for deterministic, reproducible ordering.
