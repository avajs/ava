...
class Runner extends Emittery {
			...
			if (metadata.todo) {
				...
				for (const implementation of implementations) {
					...
					if (task.metadata.skipped) {
						this.skippedTasks.push(task.title);
					}
	...
	ensureSnapshots() {
		if (!this.snapshots) {
			...
			this.snapshots.setOrdering(this.taskOrder);
			for (const title of this.skippedTasks) {
				this.snapshots.skipBlock(title);
			}
	...
	saveSnapshotState() {
		...
		if (this.updateSnapshots && this.skippedTasks.length === 0) {
