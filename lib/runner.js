...
class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.runOnlySelectedLines = this.lineNumbers.length > 0;
		...
		this.testLineNumbersCache = this.runOnlySelectedLines ? buildTestLineNumbersCache(this.file) : new Map();
		...
		this.chain = createChain((metadata, testArgs) => { // eslint-disable-line complexity
			...
			if (this.runOnlySelectedLines) {
				const {startLineNumber, startColumnNumber} = resolveTestPosition(this.file);
				const testLineNumbers = this.testLineNumbersCache.get(`${startLineNumber}:${startColumnNumber}`);
				metadata.selected = isSelectedByLineNumbers(testLineNumbers, this.lineNumbers);
			}
