...
const {normalizeFileForMatching} = require('./globs');
...
class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.runOnlySelectedLines = options.runOnlySelectedLines === true;
		...
		this.lineNumbers = this.runOnlySelectedLines ? getApplicableLineNumbers(normalizeFileForMatching(options.projectDir, options.file), options.filter) : [];
		...
		this.chain = createChain((metadata, testArgs) => { // eslint-disable-line complexity
			...
			if (this.runOnlySelectedLines) {
				const testLineNumberRange = getLineNumberRangeForTestInFile(rawTitle, this.file);
				metadata.selected = isTestSelectedByLineNumbers(testLineNumberRange, this.lineNumbers);
			}
	...
	async start() {
		...
		for (const task of this.tasks.serial) {
			...
			if (this.runOnlySelectedLines && !task.metadata.selected) {
				continue;
			}
		...
		const hasRunnableTests = concurrentTests.length > 0 || serialTests.length > 0;

		const hasSelectedTests = hasRunnableTests || todoTests.length > 0;
		if (this.runOnlySelectedLines && !hasSelectedTests) {
			this.emit('error', new Error('No tests selected by line numbers.'));
			return;
		}
