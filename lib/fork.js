...
module.exports = (file, options, execArgv) => {
	let finished = false;
	...
	const args = [options.color ? '--color' : '--no-color'].concat(options.workerArgv);

	const subprocess = childProcess.fork(workerPath, args, {
		cwd: options.projectDir,
		silent: true,
		env: {NODE_ENV: 'test', ...process.env, ...options.environmentVariables, AVA_PATH, NODE_PATH},
