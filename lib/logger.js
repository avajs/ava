...
x.report = function (passed, failed, unhandled, uncaught, startTime) {
	var duration = new Date() - startTime;
	var dur = chalk.gray.dim(' (' + prettyMs(duration) + ')');

	if (failed > 0) {
		log.writelpad(chalk.red(failed, plur('test', failed), 'failed', dur));
	} else {
		log.writelpad(chalk.green(passed, plur('test', passed), 'passed', dur));
	}
