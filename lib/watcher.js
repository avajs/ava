...
async function * readLines(stream) {
	...
	while (true) {
		const {promise, resolve: loopResolve} = PromiseWithResolvers();
		resolve = loopResolve
		yield await promise;
	}
}
	...
	eachLine(lineReader, async line => {
		...
		switch (line.toLowerCase()) {
			...
			case 'g': {
				...
				reporter.lineWriter.writeLine();
				if (promptValue === cancel || (promptValue === undefined && oldGlobPattern === undefined)) {
					signalChanged();
					break;
				}
			...
			case 'm': {
				...
				respondToChanges = true;
				reporter.lineWriter.writeLine();
				if (promptValue === cancel || (promptValue === undefined && oldMatchPattern === undefined)) {
					signalChanged();
					break;
				}
