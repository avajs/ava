	...
	const debounce = setTimeout(() => {
		...
		const changes = [...dirtyPaths].filter(path => {
			...
			for (const {main} of providers.withWatchModeSupport) {
				switch (main.interpretChange(path)) {
			...
			for (const {main} of providers.withWatchModeSupport) {
				let rewritten = main.rewriteForChangeDetection(nodePath.join(projectDir, path));
				if (rewritten !== null) {
					if (Array.isArray(rewritten)) {
					// TODO: Handle multiple possibilities for file deletions.
						debug('Change of %s could be due to deletion of multiple source files %j', path, rewritten);
						rewritten = rewritten[0];
					}

					const newPath = nodePath.relative(projectDir, rewritten);
		...
		// In the background, update the file trace to reflect the changes.
		if (changes.length > 0) {
...
class FileTracer {
	...
	async * files() {
		while (true) {
			...
			await this.#nextUpdate; // eslint-disable-line no-await-in-loop
