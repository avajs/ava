...
export default class Watcher {
		...
		this.run = (specificFiles = [], updateSnapshots = false) => {
			...
			if (specificFiles.length > 0) {
				...
				if (runOnlyExclusive) {
					...
					const remainingFiles = specificFiles.filter(file => !this.filesWithExclusiveTests.includes(file));
	...
	runAfterChanges() {
		...
		const unlinkedTests = dirtyTests.filter(path => dirtyStates[path] === 'unlink');
		...
		this.run([...new Set([...addedOrChangedTests, ...testsByHelpersOrSource.flat()])]);
	}
}
