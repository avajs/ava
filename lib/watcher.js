...
async function * plan({
	...
	eachLine(lineReader, async line => {
		switch (line.toLowerCase()) {
			...
			case 'a': {
				runAll = true;
				interactiveGlobPattern = undefined;
				interactiveMatchPattern = undefined;
				signalChanged();
				break;
			}
			...
			case 'm': {
				...
				interactiveMatchPattern = await promptForMatchPattern(reporter, lineReader, interactiveMatchPattern);
				respondToChanges = true;
				reporter.lineWriter.writeLine();
				if (oldMatchPattern === interactiveMatchPattern) {
					debounce.refresh();
				} else {
					signalChanged();
				}
