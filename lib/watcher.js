...
const endMessage = skipTests =>
	chalk.gray(
		`Type \`p\` and press enter to filter by a filename regex pattern${os.EOL}`
			+ (skipTests.fileRegexOrNull === null
				? ''
				: `   Current filename filter is ${skipTests.fileRegexOrNull}${os.EOL}`)
			+ `Type \`t\` and press enter to filter by a test name regex pattern${os.EOL}`
			+ (skipTests.testRegexOrNull === null
				? ''
				: `   Current test filter is ${skipTests.testRegexOrNull}${os.EOL}`)
			+ os.EOL
			+ (skipTests.hasAnyFilters()
				? `Type \`a\` and press enter to rerun *all* tests${os.EOL}${os.EOL}`
					+ `Type \`r\` and press enter to rerun tests that match your filters${os.EOL}`
				: `Type \`r\` and press enter to rerun tests${os.EOL}`)
			+ `Type \`u\` and press enter to update snapshots${os.EOL}`
			+ os.EOL,
	);

/**
 * Once mainstream adoption
 * of Promise.withResolvers
 * is achieved, simplify this.
 *
 */
...
export async function start({api, filter, globs, projectDir, providers, reporter, stdin, signal}) {
	...
	for await (const {files, ...runtimeOptions} of plan({
		...
		reporter.lineWriter.writeLine(
			endMessage(runtimeOptions.watchModeSkipTestsOrUndefined),
		);
		reporter.lineWriter.write(chalk.white('command > '));
	}
}
...
async function * plan({
	...
	const watchModeSkipTests = new WatchModeSkipTests();
	 ...
	 * The onDataGenerator function
	 * is an async generator that listens
	 * for lines of data from stdin.
	 *
	 * We store this generator in the field
	 * #data which we listen to in both
	 * the #listenForCommand function,
	 * and the promptForFilter. By keeping
	 * one generator instance for both
	 * we can have nested listeners.
	 */
	const _class = new (class {
		...
		#onDataGenerator = async function * () {
			let p = promiseWithResolvers();
			/**
			 * Pending data allows the data
			 * callback to be called multiple
			 * times before the promise
			 * wakes up from its await.
			 */
			const pendingData = [];
			...
			while (true) {
				/**
				 * Loops depend on each other, it makes
				 * sense to disable the eslint rule
				 */
				await p.promise; // eslint-disable-line no-await-in-loop
				p = promiseWithResolvers();
				while (pendingData.length > 0) {
					const lines = pendingData.shift().trim().split('\n');
		...
		#onCommand = async data => {
			...
			switch (data) {
				...
				case 'p': {
					watchModeSkipTests.replaceFileRegex(
						await this.#promptForFilter(
							watchModeSkipTests.fileRegexOrNull,
							'filename',
						),
					);
					reporter.lineWriter.write(`${os.EOL}${os.EOL}`);
					reporter.lineWriter.writeLine(endMessage(watchModeSkipTests));
					reporter.lineWriter.write(chalk.white('command > '));
					runSelected = true;
					break;
				}
				...
				case 't': {
					watchModeSkipTests.replaceTestRegex(
						await this.#promptForFilter(
							watchModeSkipTests.testRegexOrNull,
							'test',
						),
					);
			...
			if (runAll || runSelected || updateSnapshots) {
				signalChanged({});
			}
		};
		#promptForFilter = async (currentFilterRegexOrNull, filterName) => {
			reporter.lineWriter.writeLine(
				chalk.gray(
					`${os.EOL}${os.EOL}Current ${filterName} filter is ${
						currentFilterRegexOrNull === null
							? 'empty'
							: currentFilterRegexOrNull
					}${os.EOL}`,
				),
			);
			reporter.lineWriter.writeLine(
				chalk.gray(
					`Type the ${filterName} regex pattern then press enter:${os.EOL}`
						+ ` - Leave it blank and press enter to clear the filter${os.EOL}`,
				),
			);
			reporter.lineWriter.write('pattern > ');
			const generatorResult = (await this.#data.next());
			const patternInput = generatorResult.value.trim();
			let outFilter = null;
			if (patternInput !== '') {
				try {
					...
					outFilter = pattern;
					reporter.lineWriter.writeLine(
						chalk.gray(`Added ${filterName} regex filter: ${pattern}${os.EOL}`),
					);
				} catch (error) {
					reporter.lineWriter.writeLine(
	...
	while (abortSignal?.aborted !== true) {
		...
		const instructions = {
			...
			watchModeSkipTestsOrUndefined: runAll
				? new WatchModeSkipTests()
				: watchModeSkipTests,
		};
