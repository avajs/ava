...
const reportEndMessage = (reporter, interactiveFilter) => {
	const writeLine = message => {
		reporter.lineWriter.writeLine(chalk.gray(message));
	};

	writeLine('Type `p` and press enter to filter by a filepath regex pattern');

	const filePathRegex = interactiveFilter.printFilePathRegex();
	if (filePathRegex) {
		writeLine(`   ${filePathRegex}`);
	}

	writeLine('Type `t` and press enter to filter by a test title regex pattern');

	const testTitleRegex = interactiveFilter.printTestTitleRegex();
	if (testTitleRegex) {
		writeLine(`   ${testTitleRegex}`);
	}

	writeLine(os.EOL);
	if (interactiveFilter.hasAnyFilters()) {
		writeLine('Type `a` and press enter to rerun *all* tests');
		writeLine('Type `r` and press enter to rerun tests that match your filters');
	} else {
		writeLine('Type `r` and press enter to rerun tests');
	}

	writeLine('Type `u` followed by enter to update snapshots in selected tests');
	writeLine(os.EOL);
...
async function * plan({
	...
	const data = (async function * () {
		for await (const line of stdin.pipe(split2())) {
			yield line.trim();
		}
	})();

	const promptForFilter = async (currentFilterRegexOrNull, filterName) => {
		...
		const generatorResult = (await data.next());
	...
	const onCommand = async data => {
		...
		switch (data) {
			...
			case 'p': {
				...
				reportEndMessage(reporter, interactiveFilter);
				runSelected = filterHasChanged;
				break;
			}
