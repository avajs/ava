...
const writeCommandInstructions = (reporter, interactiveGlobPattern, interactiveFilter) => {
	...
	if (interactiveGlobPattern || interactiveFilter.hasAnyFilters()) {
...
const promptForGlobPattern = async (reporter, lineReader, currentPattern, projectDir) => {
	...
	const {value: pattern} = await lineReader.next();
	if (pattern === '') {
		return undefined;
	}

	return normalizePattern(nodePath.relative(projectDir, nodePath.resolve(process.cwd(), pattern)));
};
...
async function * plan({
	...
	const testFileSelector = (allTestFiles, selectedFiles = []) => {
		...
		if (interactiveGlobPattern !== undefined) {
	...
	eachLine(lineReader, async line => {
		switch (line.toLowerCase()) {
			...
			case 'g': {
				...
				interactiveGlobPattern = await promptForGlobPattern(reporter, lineReader, interactiveGlobPattern, projectDir);
				respondToChanges = true;
				reporter.lineWriter.writeLine();
				if (oldGlobPattern === interactiveGlobPattern) {
					debounce.refresh();
				} else {
					signalChanged();
				}
	...
	while (abortSignal?.aborted !== true) {
		...
		const instructInteractiveFilter = runAll ? new InteractiveFilter() : interactiveFilter;
		...
		yield {
			...
			interactiveFilter: instructInteractiveFilter,
		};
		respondToChanges = true;

		// Write command instructions after the tests have run and been reported.
		writeCommandInstructions(reporter, interactiveGlobPattern, interactiveFilter);
