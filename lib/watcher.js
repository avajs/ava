...
class Watcher {
	...
	trackTestDependencies(api) {
		...
		api.on('run', plan => {
			plan.status.on('stateChange', evt => {
				...
				const sourceDeps = evt.dependencies.map(x => relative(x)).filter(filePath => globs.classify(filePath, this.globs).isSource);
	...
	runAfterChanges() {
		...
		const dirtyTests = dirtyPaths.filter(filePath => globs.classify(filePath, this.globs).isTest);
