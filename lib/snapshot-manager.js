...
function determineSourceMappedDir(options) {
	...
	const sourceMapConverted = convertSourceMap.fromSource(sourceMapContent) ||
		convertSourceMap.fromMapFileSource(
			sourceMapContent, path.dirname(sourceMapFilePath)
		);
	if (sourceMapConverted && sourceMapConverted.sourcemap) {
		const sourcemap = sourceMapConverted.sourcemap;
		const firstSource = sourcemap.sources[0];
		const sourceRoot = sourcemap.sourceRoot === '' ?
			'../' : sourcemap.sourceRoot;
		const rootedSource = sourceRoot ?
			sourceRoot + firstSource : firstSource;
