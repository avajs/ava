...
function combineEntries(snapshotData) {
	const combined = new Buffers();

	for (const {title, snapshots} of snapshotData.blocks) {
...
function generateReport(relFile, snapFile, snapshotData) {
...
Generated by [AVA](https://avajs.dev).`);
	...
	report.append(combineEntries(snapshotData));
...
function encodeSnapshots(snapshotData) {
	const buffer = cbor.encodeOne(snapshotData, {
...
class Manager {
	...
	save() {
		...
		const buffer = encodeSnapshots(snapshotData);
		const reportBuffer = generateReport(relFile, snapFile, snapshotData);
