...
class Manager {
	...
	recordSerialized({buffer, label, belongsTo, index}) {
	...
	deferRecord(options, changed = true) {
		...
		return () => { // Must be called in order!
			...
			this.recordSerialized({buffer, label, belongsTo, index});
		};
	}
	...
	skipSnapshot({belongsTo, index, deferRecording}) {
		...
		if (snapshot) {
			...
			if (deferRecording) {
				return () => this.recordSerialized({belongsTo, index, ...snapshot});
			}

			this.recordSerialized({belongsTo, index, ...snapshot}, false);
		}
	}
...
function lazy(loader) {
	...
	function createBufferFunction(property) {
		buffers[property] = [];
		return (...args) => {
			if (manager === null) {
				buffers[property].push(args);
			} else {
				manager[property](...args);
			}
		};
	}
	...
	function ensure() {
		if (manager === null) {
			manager = loader();
			for (const property of Object.keys(buffers)) {
				for (const args of buffers[property]) {
					manager[property](...args);
				}
			}
		}
	...
	return {
		compare(...args) {
			...
			return manager.compare(...args);
		},
		skipBlock: createBufferFunction('skipBlock'),
		skipSnapshot({deferRecording, ...options}) {
			if (manager !== null) {
				return manager.skipSnapshot({deferRecording, ...options});
			}

			if (deferRecording) {
				return () => this.skipSnapshot(options);
			}

			buffers.skipSnapshot.push([options]);
		},
		setOrdering: createBufferFunction('setOrdering'),
		save(...args) {
			if (manager !== null) {
				return manager.save(...args);
			}
