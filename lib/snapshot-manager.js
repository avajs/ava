...
function combineEntries(blocksByTitle) {
	...
	for (const [title, {snapshots}] of sortBlocks(blocksByTitle)) {
...
Generated by [AVA](https://avajs.dev).`);
	...
	report.append(combineEntries(blocksByTitle));
...
function encodeSnapshots(blocksByTitle) {
	const data = {
		blocks: sortBlocks(blocksByTitle).map(
			([title, {taskIndex, ...block}]) => ({title, ...block})
		)
	};
	...
	return Buffer.concat([
		...
		sha256sum,
		compressed
	], READABLE_PREFIX.byteLength + VERSION_HEADER.byteLength + sha256sum.byteLength + compressed.byteLength);
}
