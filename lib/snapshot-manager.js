...
function combineEntries(entries, ordering) {
	const buffers = [];
	let byteLength = 0;

	const sortedKeys = [...entries.keys()].sort((a, b) => {
		const [taskIndexA, snapIndexA] = ordering.get(a);
		const [taskIndexB, snapIndexB] = ordering.get(b);
		const taskDifference = taskIndexA - taskIndexB;

		if (taskDifference !== 0) {
			return taskDifference;
		}

		const snapDifference = snapIndexA - snapIndexB;
		return snapDifference;
	});
...
function generateReport(relFile, snapFile, entries, ordering) {
	const combined = combineEntries(entries, ordering);
...
function appendReportEntries(existingReport, entries, ordering) {
	const combined = combineEntries(entries, ordering);
	const {buffers} = combined;
	let {byteLength} = combined;
...
function encodeSnapshots(buffersByHash) {
	...
	for (const pair of sortedBuffersByHash) {
		const hash = pair[0];
...
class Manager {
	...
	deferRecord(hash, options) {
		...
		const {taskIndex, snapIndex} = options;
		...
		return () => { // Must be called in order!
			...
			if (!this.reportOrder.has(options.belongsTo)) {
				this.reportOrder.set(options.belongsTo, [taskIndex, snapIndex]);
			}
		};
	}
	...
	save() {
		...
		const reportBuffer = existingReport ?
			appendReportEntries(existingReport, this.reportEntries, this.reportOrder) :
			generateReport(this.relFile, this.snapFile, this.reportEntries, this.reportOrder);
