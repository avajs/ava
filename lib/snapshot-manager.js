...
function combineEntries(blocksByTitle, blockIndices) {
	const combined = new Buffers();

	for (const [title, {snapshots}] of sortBlocks(blocksByTitle, blockIndices)) {
...
function generateReport(relFile, snapFile, blocksByTitle, blockIndices) {
...
Generated by [AVA](https://avajs.dev).`);
	...
	report.append(combineEntries(blocksByTitle, blockIndices));
...
function encodeSnapshots(blocksByTitle, blockIndices) {
	const data = {
		blocks: sortBlocks(blocksByTitle, blockIndices).map(
...
class Manager {
	...
	compare(options) {
		this.touch(options.belongsTo, options.taskIndex);
	...
	skipSnapshot({belongsTo, index, deferRecording, taskIndex}) {
		this.touch(belongsTo, taskIndex);
	...
	save() {
		...
		const buffer = encodeSnapshots(this.newBlocksByTitle, this.blockIndices);
		const reportBuffer = generateReport(relFile, snapFile, this.newBlocksByTitle, this.blockIndices);
