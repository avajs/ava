...
class Manager {
	constructor(options) {
		...
		this.ordering = options.ordering || [...options.blocksByTitle.keys()];
	...
	save() {
		...
		const {snapPath} = this;
		const buffer = encodeSnapshots(this.blocksByTitle, this.ordering);

		const reportPath = path.join(this.dir, this.reportFile);
		const reportBuffer = generateReport(this.relFile, this.snapFile, this.blocksByTitle, this.ordering);
