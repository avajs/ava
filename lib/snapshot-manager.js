...
function combineEntries(entries) {
	...
	const sortedKeys = [...entries.keys()].sort((kA, kB) => {
		const [a, b] = [entries.get(kA), entries.get(kB)];
	...
	deferRecord(hash, options) {
		...
		return () => { // Must be called in order!
			...
			if (this.reportEntries.has(options.belongsTo)) {
				...
				this.reportEntries.set(options.belongsTo, {buffers: [entry], taskIndex, snapIndex});
			}
		};
	}
