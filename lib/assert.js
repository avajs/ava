...
x.notThrow = function (fn, msg) {
	...
	try {
		...
		test(false, create(err.actual, err.expected, err.operator, err.message, x.notThrow));
	}
};

x.doesNotThrow = util.deprecate(x.notThrow, 't.doesNotThrow is renamed to t.notThrow. The old name still works, but will be removed in AVA 1.0.0. Update your references.');
