...
class Assertions {
	constructor({
		...
		const snapshotArgs = (expected, ...rest) => {
			let message;
			let snapshotOptions;
			if (rest.length > 1) {
			...
			const id = snapshotOptions ? snapshotOptions.id : undefined;
		...
		this.snapshot = (...args) => {
			...
			const {expected, message, id} = snapshotArgs(...args);
			...
			let result;
			try {
				result = compareWithSnapshot({expected, id, message});
			} catch (error) {
		...
		this.snapshot.skip = (...args) => {
			skipSnapshot(snapshotArgs(...args));
		};
