...
class Assertions {
	constructor({
		...
		const _skippable = assertionFn => {
			assertionFn.skip = (...args) => skip(...args);
			return assertionFn;
		};

		const _enhanced = (pattern, assertionFn) => {
		 ...
		 * @public
		 */
		this.is = _skippable((actual, expected, message) => {
		 ...
		 * @public
		 */
		this.not = _skippable((actual, expected, message) => {
			if (Object.is(actual, expected)) {
				fail(new AssertionError({
					assertion: 'not',
					message,
		 ...
		 * @public
		 */
		this.deepEqual = _skippable((actual, expected, message) => {
			const result = concordance.compare(actual, expected, concordanceOptions);
			if (result.pass) {
				pass();
			} else {
		 ...
		 * @public
		 */
		this.notDeepEqual = _skippable((actual, expected, message) => {
			const result = concordance.compare(actual, expected, concordanceOptions);
			if (result.pass) {
				const actualDescriptor = result.actual || concordance.describe(actual, concordanceOptions);
				fail(new AssertionError({
		...
		this.throwsAsync = _skippable((...args) => {
		...
		this.notThrows = _skippable((fn, message) => {
			if (typeof fn !== 'function') {
				fail(new AssertionError({
					assertion: 'notThrows',
					improperUsage: true,
		...
		this.notThrowsAsync = _skippable((nonThrower, message) => {
		...
		this.snapshot = _skippable((expected, optionsOrMessage, message) => {
			const options = {};
		...
		this.truthy = _skippable((actual, message) => {
			if (actual) {
				pass();
			} else {
				fail(new AssertionError({
		...
		this.falsy = _skippable((actual, message) => {
			if (actual) {
				fail(new AssertionError({
					assertion: 'falsy',
					message,
		...
		this.true = _skippable((actual, message) => {
			if (actual === true) {
				pass();
			} else {
				fail(new AssertionError({
		...
		this.false = _skippable((actual, message) => {
			if (actual === false) {
				pass();
			} else {
				fail(new AssertionError({
		...
		this.assert = _skippable(_enhanced(
