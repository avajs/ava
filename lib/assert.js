...
x.snapshot = function (tree, snapshotLib, globalsOptions) {
	// set defaults - this allows tests to mock deps easily
	var snapshot = snapshotLib || jestSnapshot;
	...
	var snapshotState = snapshot.initializeSnapshotState(
    filename,
	...
	var result = snapshot.toMatchSnapshot.call(context, tree);

	var messagePrefix = 'Please check your code or --update-snapshots\n\n';
	var message = typeof result.message === 'function' && (messagePrefix + result.message());

	snapshotState.save();

	test(result.pass, create(result, false, 'snap', message, x.snap));
};
