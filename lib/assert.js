...
// Gets part of an object based on another object used as a template for getting properties.
function getObjectSubset(template, target, templateValueNodes = []) {
	if (!template || !target || (typeof template !== 'object') || (typeof target !== 'object')) {
		return target;
	}

	// FIXME: Too 'clever'?
	const [result, entries] = Array.isArray(template) ?
		[[], template.entries()] :
		[{}, Object.entries(template)];

	for (const [templateKey, templateValue] of entries) {
		if (templateValueNodes.includes(templateValue)) {
			result[templateKey] = target[templateKey];
			break;
		}

		const nextTemplateValueNodes = [...templateValueNodes, templateValue];
		result[templateKey] = getObjectSubset(templateValue, target[templateKey], nextTemplateValueNodes);
	}
...
class Assertions {
	constructor({
		...
		this.matches = withSkip((actual, expected, message) => {
			if (!checkMessage('matches', message)) {
				return;
			}

			const actualSubset = getObjectSubset(expected, actual);

			const result = concordance.compare(actualSubset, expected, concordanceOptions);
			if (result.pass) {
				pass();
			} else {
				const actualDescriptor = result.actual || concordance.describe(actual, concordanceOptions);
