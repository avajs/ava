...
// Gets a part of an object based on another object used as a template for deep properties.
function getPartialObject(templateObject, target) {
  if (!templateObject || !target || (typeof templateObject !== 'object') || (typeof target !== 'object')) {
  ...
  for (const key of keys) {
    if (templateObject[key]) {
      result[key] = getPartialObject(target[key], templateObject[key]);
    }
  }
...
class Assertions {
	constructor({
		...
		this.matches = withSkip((actual, expected, message) => {
			...
			const actualPartial = getPartialObject(expected, actual);

			const result = concordance.compare(actualPartial, expected, concordanceOptions);
			if (result.pass) {
				pass();
			} else {
				const actualDescriptor = result.actual || concordance.describe(actual, concordanceOptions);
