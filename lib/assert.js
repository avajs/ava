...
const likePatternConcordance = require('./like-pattern-concordance');
...
class Assertions {
	constructor({
		...
		this.like = withSkip((actual, likePattern, message) => {
			if (!checkMessage('like', message)) {
				return;
			}

			if (isPlainObject(likePattern) && Object.keys(likePattern).length === 0) {
				fail(new AssertionError({
					assertion: 'like',
					improperUsage: true,
					message: '`t.like()` pattern must not be an empty object',
					values: [formatWithLabel('Called with:', likePattern)]
				}));
				return;
			}

			const {actualPicked, result} = likePatternConcordance.compare(actual, likePattern, concordanceOptions);
			if (result.pass) {
				pass();
			} else {
				const actualDescriptor = result.actual || concordance.describe(actualPicked, concordanceOptions);
				const expectedDescriptor = result.expected || concordance.describe(likePattern, concordanceOptions);
				fail(new AssertionError({
					assertion: 'like',
					message,
					values: [formatDescriptorDiff(actualDescriptor, expectedDescriptor)]
