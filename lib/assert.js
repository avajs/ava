...
function validateExpectations(assertion, expectations) { // eslint-disable-line complexity
	...
	} else if (expectations === null || expectations === undefined) {
		expectations = {};
...
class Assertions {
	...
	/**
	 * @param { {expected, [id]: string, [message]: string} } options
	 * @returns { {pass: boolean, [actual], [expected]} }
	 * @protected
	 */
	_compareWithSnapshot() {
		throw new Error('not implemented');
	}
	 ...
	 * @private
	 */
	_skippable(assertionFn) {
		assertionFn.skip = (...args) => this._skip(...args);
		return assertionFn;
	}
	...
	_enhanced(pattern, assertionFn) {
		...
		return empower(assertionFn, empowerOptions);
	}
	...
	constructor() {
		 ...
		 * @public
		 */
		this.fail = this._skippable(message => {
			this._fail(new AssertionError({
				assertion: 'fail',
		 ...
		 * @public
		 */
		this.is = this._skippable((actual, expected, message) => {
		 ...
		 * @public
		 */
		this.not = this._skippable((actual, expected, message) => {
			if (Object.is(actual, expected)) {
				this._fail(new AssertionError({
					assertion: 'not',
					message,
		 ...
		 * @public
		 */
		this.deepEqual = this._skippable((actual, expected, message) => {
			const result = concordance.compare(actual, expected, concordanceOptions);
			if (result.pass) {
				this._pass();
			} else {
		 ...
		 * @public
		 */
		this.notDeepEqual = this._skippable((actual, expected, message) => {
			const result = concordance.compare(actual, expected, concordanceOptions);
			if (result.pass) {
				const actualDescriptor = result.actual || concordance.describe(actual, concordanceOptions);
				this._fail(new AssertionError({
		...
		this.throws = this._skippable((fn, expectations, message) => {
			...
			try {
				expectations = validateExpectations('throws', expectations);
			} catch (error) {
				this._fail(error);
				return;
			}
			...
			try {
				...
				if (isPromise(retval)) {
					try {
						// In reality isPromise returns if something is PromiseLike
						// i.e. has a .then() function. Wrapping in Promise.resolve()
						// casts it to actual promise
		...
		this.throwsAsync = this._skippable((thrower, expectations, message) => {
			...
			try {
				expectations = validateExpectations('throwsAsync', expectations);
			} catch (error) {
				this._fail(error);
				return Promise.resolve();
			}
			...
			const handlePromise = (promise, wasReturned) => {
				...
				// Wrap in Promise.resolve to cast PromiseLike into real Promise
		...
		this.notThrows = this._skippable((fn, message) => {
			if (typeof fn !== 'function') {
				this._fail(new AssertionError({
					assertion: 'notThrows',
					improperUsage: true,
		...
		this.notThrowsAsync = this._skippable((nonThrower, message) => {
			...
			const handlePromise = (promise, wasReturned) => {
				// Record stack before it gets lost in the promise chain.
				const stack = getStack();
				const intermediate = Promise.resolve(promise).then(noop, reason => {
					throw new AssertionError({
						assertion: 'notThrowsAsync',
						message,
						actualStack: stack,
		...
		this.snapshot = this._skippable((expected, optionsOrMessage, message) => {
			const options = {};
			...
			let result;
			try {
				result = this._compareWithSnapshot(options);
			} catch (error) {
		...
		this.truthy = this._skippable((actual, message) => {
			if (actual) {
				this._pass();
			} else {
				this._fail(new AssertionError({
		...
		this.falsy = this._skippable((actual, message) => {
			if (actual) {
				this._fail(new AssertionError({
					assertion: 'falsy',
					message,
		...
		this.true = this._skippable((actual, message) => {
			if (actual === true) {
				this._pass();
			} else {
				this._fail(new AssertionError({
		...
		this.false = this._skippable((actual, message) => {
			if (actual === false) {
				this._pass();
			} else {
				this._fail(new AssertionError({
		...
		this.assert = this._skippable(this._enhanced(
