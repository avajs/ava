		...
		if(Array.isArray(ref)){
			for (const extension of extensionsToLoadAsModules) {
				if (ref[0].endsWith(`.${extension}`)) {
					...
					if (isESMSupported) {
						return import(pathToFileURL(ref[0])); // eslint-disable-line node/no-unsupported-features/es-syntax
					}
			...
			for (const provider of providers) {
				if (provider.canLoad(ref[0])) {
					return provider.load(ref[0], {requireFn});
				}
			}
	
			return requireFn(ref[0])(ref[1]);
