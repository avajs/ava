	...
	const load = async ref => {
		for (const extension of extensionsToLoadAsModules) {
			if (ref.endsWith(`.${extension}`)) {
				if (hasDynamicImport) {
					return await import(pathToFileURL(ref)); // eslint-disable-line no-return-await, no-await-in-loop
				}

				ipc.send({type: 'internal-error', err: serializeError('Internal runner error', false, new Error('ESM files cannot be loaded in this version of Node.js.'))});
				exit(1);
				return;
			}
		}
