...
class IPCHandle extends Handle {
	constructor() {
		super();
		if (!controlFlow) {
			controlFlow = require('../ipc-flow-control');
		}
...
function createHandleFromMessagePort(port) {
	handle.channel = parentPort;
	handle.ref = parentPort;
	handle.send = evt => {
		handle.channel.postMessage({ava: evt});
...
function createHandleFromIPC() {}
function sharedWorkerChannel(port, filename, initialData) {
	const channelId = `${getOptions().forkId}/channel/${++channelCounter}`;
	const [channelEmitter, unsubscribe] = createChannelEmitter(channelId);
...
exports.sharedWorkerChannel = sharedWorkerChannel;
