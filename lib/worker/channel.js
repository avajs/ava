...
if (isRunningInChildProcess) {
	...
	channel = process;
	_ref = process.channel;
} else if (isRunningInThread) {
	const {workerData} = require('worker_threads');
	channel = workerData.port;
	_ref = workerData.port;
	send = evt => {
		channel.postMessage({ava: evt});
	};
}

exports.options = pEvent(channel, 'message', selectAvaMessage('options')).then(message => message.ava.options);
exports.peerFailed = pEvent(channel, 'message', selectAvaMessage('peer-failed'));
exports.send = send;
...
async function flush() {
	...
	const promise = pendingPings.then(async () => { // eslint-disable-line promise/prefer-await-to-then
		send({type: 'ping'});
		await pEvent(channel, 'message', selectAvaMessage('pong'));
