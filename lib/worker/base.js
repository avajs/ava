...
// Resolve a TypeScript file to its compiled JavaScript output using the
// rewritePaths from the @ava/typescript provider state. Returns null if the
// file is not handled by a provider.
function resolveCompiledFile(file, providerStates) {
	for (const {type, state} of providerStates) {
		if (type !== 'typescript') {
			continue;
		}

		const {extensions = ['ts', 'cts', 'mts'], rewritePaths = []} = state;
		const testFileExtension = new RegExp(`\\.(${extensions.join('|')})$`);

		if (!testFileExtension.test(file)) {
			continue;
		}

		const match = rewritePaths.find(([from]) => file.startsWith(from));
		if (!match) {
			continue;
		}

		const [from, to] = match;
		let rewritten = `${to}${file.slice(from.length)}`;
...
const run = async options => {
	...
	// Resolve the compiled file for TypeScript sources so that acorn can parse
	// the JavaScript output instead of the TypeScript source. This is needed
	// because acorn does not support TypeScript syntax.
	const compiledFile = resolveCompiledFile(options.file, options.providerStates ?? []);
