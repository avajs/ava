...
import {MAX_SEED} from './random-order.js';
...
const FLAGS = {
	...
	seed: {
		coerce: coerceLastValue,
		description: 'Seed the random order of test files, so it can be reproduced (implies --randomize)',
		type: 'number',
	},
	serial: {
		alias: 's',
...
export default async function loadCli() { // eslint-disable-line complexity
	...
	if (Object.hasOwn(combined, 'seed') && (!Number.isInteger(combined.seed) || combined.seed < 0 || combined.seed > MAX_SEED)) {
		exit(`The --seed flag must be provided with an integer between 0 and ${MAX_SEED}.`);
	}

	if (combined.randomize === false && Object.hasOwn(combined, 'seed')) {
		exit('The --seed flag must not be combined with --no-randomize.');
	}

	const randomizeFileOrder = combined.randomize === true || (combined.randomize !== false && Object.hasOwn(combined, 'seed'));

	if (randomizeFileOrder && Object.hasOwn(conf, 'sortTestFiles')) {
		exit('’sortTestFiles’ must not be configured when randomizing the test file order.');
	}
	...
	if (randomizeFileOrder && parallelRuns !== null && !Object.hasOwn(combined, 'seed')) {
		exit('When utilizing parallel builds, --randomize requires an explicit --seed so all builds order the test files the same way.');
	}
