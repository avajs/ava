...
exports.run = async () => { // eslint-disable-line complexity
	...
	if (typeof combined.workerThreads !== 'boolean') {
		combined.workerThreads = true;
	}

	for (const flag of Object.keys(FLAGS)) {
		if (flag === 'no-worker-threads' && typeof argv['worker-threads'] === 'boolean') {
	...
	const api = new Api({
		...
		workerThreads: combined.WorkerThreads,
		debug,
