...
exports.run = async () => { // eslint-disable-line complexity
	...
	let extensions;
	try {
		extensions = normalizeExtensions(conf.extensions, babelProvider, {experiments});
	} catch (error) {
		exit(error.message);
	}
	...
	const files = cli.input.map(file =>
		path.relative(resolveTestsFrom, path.resolve(process.cwd(), stripLineNumbersSuffix(file)))
	);
	...
	const lineNumbers = {};
	for (const [index, file] of cli.input.entries()) {
		const triesToRunFileWithAndWithoutLineNumbers = cli.input
			.filter(f => f === file)
			.some(f =>
				hasLineNumbersSuffix(f) && !hasLineNumbersSuffix(file) ||
				!hasLineNumbersSuffix(f) && hasLineNumbersSuffix(file)
			);
		if (triesToRunFileWithAndWithoutLineNumbers) {
			exit(`Can't run ${file} with and without selected line numbers.`);
		}

		if (hasLineNumbersSuffix(file)) {
			const absolutePath = path.resolve(resolveTestsFrom, files[index]);
			lineNumbers[absolutePath] = lineNumbers[absolutePath] || [];
			lineNumbers[absolutePath] = [...new Set([...lineNumbers[absolutePath], ...parseLineNumbersInPath(file)])];
		}
	}
	...
	if (conf.watch) {
		if (Object.keys(lineNumbers).length > 0) {
			exit('Watch mode is not available when selecting individual line numbers.');
		}
