...
exports.run = async () => { // eslint-disable-line complexity
	...
	const patternsAndLineNumbers = input.map(pattern => splitPatternAndLineNumbers(pattern));
	const filter = normalizePatterns(
		patternsAndLineNumbers.map(({pattern}) => path.relative(projectDir, path.resolve(process.cwd(), pattern)))
	).map((pattern, index) => ({...patternsAndLineNumbers[index], pattern}));
