...
exports.run = async () => { // eslint-disable-line complexity
	...
	const canonicalFilePaths = cli.input.map(file => path.resolve(process.cwd(), file));
	const files = [...new Set(canonicalFilePaths.map(file =>
		path.relative(process.cwd(), stripLineNumbersSuffix(file)))
	)];
