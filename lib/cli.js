...
exports.run = async () => { // eslint-disable-line complexity
	...
	const {argv} = yargs
		...
		.usage('$0 [<pattern>[:<line-numbers>]...]')
		.usage('$0 debug [<pattern>[:<line-numbers>]...]')
		.usage('$0 reset-cache')
		.options({
			color: {
	...
	const canonicalFilePaths = input.map(fileOrPattern => path.resolve(process.cwd(), fileOrPattern));
	const filter = normalizePatterns(canonicalFilePaths.map(fileOrPattern => path.relative(projectDir, stripLineNumbersSuffix(fileOrPattern))));

	const lineNumbers = parseLineNumbersInPaths(canonicalFilePaths);
