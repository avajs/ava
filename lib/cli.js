...
exports.run = async () => { // eslint-disable-line complexity
	...
	if (combined.match && combined.updateSnapshots) {
	...
	if (filter.some(condition => condition.lineNumbers) && combined.updateSnapshots) {
		exit('Snapshots can`t be updated with a line filter.');
	}
