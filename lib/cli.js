...
exports.run = async () => { // eslint-disable-line complexity
	...
	if (
		...
		(cli.flags.concurrency && (!Number.isInteger(Number.parseFloat(cli.flags.concurrency)) || Number.parseInt(cli.flags.concurrency, 10) < 0))
	) {
	...
	const api = new Api({
		...
		concurrency: conf.concurrency ? Number.parseInt(conf.concurrency, 10) : 0,
