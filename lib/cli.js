...
exports.run = function () {
	...
	var cli = meow([
		...
		'  --extension, -e         Test file extension (e.g. `ts`) (Can be repeated)',
		'',
		'Examples',
		'  ava',
	...
	var extensions = [].concat(cli.flags.extension || 'js');
	if (extensions.some(ext => allowedExtensions.indexOf(ext) === -1)) {
		throw new Error(colors.error(figures.cross) + ' The --extension are only support: ' + allowedExtensions.join(','));
	}
	...
	if (cli.flags.watch) {
		try {
			var watcher = new Watcher(logger, api, files, arrify(cli.flags.source), extensions);
