...
const FLAGS = {
	...
	seed: {
		coerce: coerceLastValue,
		description: 'Seed for randomizing test order within files (use without value for random seed)',
		type: 'number',
	},
	serial: {
		alias: 's',
...
export default async function loadCli() { // eslint-disable-line complexity
	...
	// If --seed was passed without a value, yargs parses it as 0, NaN, or undefined depending on context.
	// Generate a random seed when the flag is present but has no meaningful value.
	if (Object.hasOwn(argv, 'seed')) {
		if (combined.seed === undefined || Number.isNaN(combined.seed)) {
			combined.seed = Math.floor(Math.random() * (2 ** 32));
		} else if (!Number.isInteger(combined.seed) || combined.seed < 0) {
			exit('The --seed flag must be a non-negative integer.');
		}
	}
