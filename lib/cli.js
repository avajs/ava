...
const FLAGS = {
	...
	watch: {
		alias: 'w',
		coerce: coerceLastValue,
		description: 'Re-run tests when tests and source files change',
		type: 'boolean'
	}
};
...
exports.run = async () => { // eslint-disable-line complexity
	...
	const chalk = require('./chalk').set({level: argv.color ? 1 : 0});
	...
	if (typeof conf.compileEnhancements === 'boolean') {
	...
	let globs;
	try {
		globs = normalizeGlobs(conf.files, conf.helpers, conf.sources, extensions);
	} catch (error) {
		exit(error.message);
	}
