...
const parseLineNumbers = suffix => flatten(suffix.split(',').map(part => {
	const trimmed = removeAllWhitespace(part);
	...
	if (match = RANGE_REGEX.exec(trimmed)) {
		...
		if (start > end) {
			throw new Error(`Invalid line number range: \`${trimmed}\`. \`start\` must be less than \`end\`.`);
		}
