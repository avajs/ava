...
const isTestSelectedByLineNumbers = ({selectedLines, allCallLocations, file}) => {
...
function initializeLineNumberSelector(file, lineNumbers = []) {
	...
	walk.simple(ast, {
		CallExpression(node) {
			allCallLocations.push(node.loc);
		}
	});

	allCallLocations.sort((a, b) => {
	...
	return {
		isSelected: () => isTestSelectedByLineNumbers({selectedLines, allCallLocations, file})
	};
}
