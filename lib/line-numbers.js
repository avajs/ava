...
function initializeLineNumberSelector(file, lineNumbers = []) {
	...
	// Record the start and end positions of all call expressions.
	...
	return {
		isSelected() {
			...
			// Now determine if any of the enclosed lines are selected.
			return enclosingCalls.some(loc => {
				for (let {line} = loc.start; line <= loc.end.line; line++) {
