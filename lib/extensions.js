export default function resolveExtensions(configuredExtensions, providers = []) {
	...
	const normalize = extensions => {
		...
		for (const key of keys) {
			if (key === 'js' || key === 'cjs' || key === 'mjs') {
				if (extensions[key] !== true) {
					throw new TypeError(`When specifying extensions, use \`true\` for \u2018${key}\u2019 extensions`);
				}

				continue;
			}

			if (extensions[key] !== 'module') {
				throw new TypeError(`When specifying extensions, the value for \u2018${key}\u2019 must be \u2018module\u2019`);
			}
		}
