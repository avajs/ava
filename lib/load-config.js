...
function findUnsupportedConfigs(searchDir) {
	const unsupportedConfig = path.join(searchDir, 'ava.config.json');
...
export async function loadConfig({configFile, resolveFrom = process.cwd(), defaults = {}, handleUnsupportedConfigs = () => {}} = {}) {
	...
	if (configFile) {
		...
		unsupportedConfigs.push(...findUnsupportedConfigs(searchDir));
		do {
	...
	handleUnsupportedConfigs(unsupportedConfigs);
	return config;
}
