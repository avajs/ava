...
function warnOnOldConfig(searchDir) {
	const v3ConfigPath = path.join(searchDir, 'ava.config.json');
	if (fs.existsSync(v3ConfigPath)) {
		console.log(chalk.magenta(`  ${figures.warning} AVA does not support JSON config anymore. Ignoring ${v3ConfigPath}`));
	}
}
