...
const importConfig = async ({configFile, fileForErrorMessage}) => {
	const {default: config = MISSING_DEFAULT_EXPORT} = await import(url.pathToFileURL(configFile).toString()); // eslint-disable-line node/no-unsupported-features/es-syntax
...
export async function loadConfig({configFile="", resolveFrom = process.cwd(), defaults = {}} = {}) {
	...
	if (fileConf !== NO_SUCH_FILE) {
		const typedFileConf = /** @type {function|object} */ (fileConf)
		...
		if (typeof typedFileConf === 'function') {
			fileConf = await typedFileConf({projectDir});
		...
		if (typeof fileConf === 'object' && 'ava' in fileConf) {
			throw new Error(`Encountered ’ava’ property in ${fileForErrorMessage}; avoid wrapping the configuration`);
		}
	}
	const typeCastedToObjectFileConf = /** @type {Object} */ (fileConf)
	const config = {...defaults, nonSemVerExperiments: {}, ...typeCastedToObjectFileConf, ...packageConf, projectDir, configFile};
