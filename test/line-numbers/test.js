...
test('no test selected by line number', async t => {
	const result = await t.throwsAsync(exec.fixture(['line-numbers.js:6']));

	t.snapshot(exec.cleanOutput(result.stdout), 'fails with message');
});

test('parent call is not selected', async t => {
	const result = await t.throwsAsync(exec.fixture(['line-numbers.js:23']));
