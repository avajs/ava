...
test('bails when using --watch while while debugging', t => {
	execCli(['debug', '--watch', 'test.js'], {dirname: 'fixture/tap', env: {AVA_FORCE_CI: 'not-ci'}}, (error, stdout, stderr) => {
...
test('bails when debugging in CI', t => {
	execCli(['debug', 'test.js'], {dirname: 'fixture/tap', env: {AVA_FORCE_CI: 'ci'}}, (error, stdout, stderr) => {
		t.equal(error.code, 1);
...
test('bails when --tap reporter is used while debugging', t => {
	execCli(['debug', '--tap', 'test.js'], {dirname: 'fixture/tap', env: {AVA_FORCE_CI: 'not-ci'}}, (error, stdout, stderr) => {
		t.equal(error.code, 1);
