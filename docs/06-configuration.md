...
This is still an experimental feature. You need to enable the `configurableModuleFormat` experiment. Then specify `extensions` using an object form. As with the array form you need to explicitly list `js`, `cjs` and `mjs` extensions, which **must** be set to the `true` value. Configure other extensions using either `'commonjs'` or `'module'`:
